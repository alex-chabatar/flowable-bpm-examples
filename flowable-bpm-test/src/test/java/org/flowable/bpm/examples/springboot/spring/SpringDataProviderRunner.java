...
public class SpringDataProviderRunner extends SpringJUnit4ClassRunner {
    ...
    /**
     * The {@link TestValidator} to be used to validate all test methods to be executed as test and all dataprovider to be
     * used to explode tests.
     */
    ...
    /**
     * Initialize and/or override {@link DataConverter}, {@link TestGenerator} and/or {@link TestValidator} helper
     * classes.
     */
     ...
     * <p>
     * Additionally, caches the result as {@link #computeTestMethods()} is call multiple times while test execution by the
     * JUnit framework (to validate, to filter, to execute, ...).
     *
     ...
     * {@inheritDoc}
     * <p>
     * If possible the given {@code filter} is wrapped by {@link DataProviderFilter} to enable filtering of tests using a
     * dataprovider.
     *
     ...
     * {@link FrameworkMethod}s is checked if it uses a {@code @}{@link DataProvider} or not. If yes, for each line of the
     * {@link DataProvider}s result a specific, parameterized test method will be added. If no, the original test method
     * is added.
     * <p>
    ...
    private FrameworkMethod findDataProviderMethod(List<FrameworkMethod> dataProviderMethods,
        ...
        if (result == null) {
            String dataProviderMethodName = "dataProvider" + toUpperCase(testMethodName.charAt(0))
                    + testMethodName.substring(1);
            result = findMethod(dataProviderMethods, dataProviderMethodName);
        }
        if (result == null) {
